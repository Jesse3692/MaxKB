...
class CompletionHistoryMessage(serializers.Serializer):
    role = serializers.CharField(required=True, label=_("Role"))
    content = serializers.CharField(required=True, label=_("content"))


class CompletionsRequestSerializer(serializers.Serializer):
    model = serializers.CharField(required=True, label=_("Model"))
    prompt = serializers.CharField(required=True, label=_("Prompt"))
    system = serializers.CharField(
        required=False, allow_blank=True, label=_("System prompt")
    )
    history = serializers.ListField(
        child=CompletionHistoryMessage(), required=False, default=list
    )
...
class CompletionChoiceSerializer(serializers.Serializer):
...
class CompletionUsageSerializer(serializers.Serializer):
    prompt_tokens = serializers.IntegerField(required=True)
    completion_tokens = serializers.IntegerField(required=True)
    total_tokens = serializers.IntegerField(required=True)


class CompletionsResponseSerializer(serializers.Serializer):
    id = serializers.CharField(required=True)
    object = serializers.CharField(required=True)
    created = serializers.IntegerField(required=True)
    model = serializers.CharField(required=True)
    system_fingerprint = serializers.CharField(
        required=False, allow_blank=True, allow_null=True
    )
    choices = CompletionChoiceSerializer(many=True, required=True)
    usage = CompletionUsageSerializer(required=True)
...
class OpenAIChatSerializer(serializers.Serializer):
    ...
    def generate_chat(chat_id, application_id, message, chat_user_id, chat_user_type):
        if chat_id is None:
            ...
            chat_info = ChatInfo(
                chat_id, chat_user_id, chat_user_type, [], [], application_id
            )
            chat_info.set_cache()
        else:
    ...
    def chat(self, instance: Dict, with_valid=True):
        ...
        chat_id = self.generate_chat(
            chat_id, application_id, message, chat_user_id, chat_user_type
        )
        return ChatSerializers(
            data={
                "chat_id": chat_id,
...
class OpenChatSerializers(serializers.Serializer):
    ...
    def open_work_flow(self, application):
        ...
        ChatInfo(
            chat_id, chat_user_id, chat_user_type, [], [], application_id, debug
        ).set_cache()
        return chat_id
